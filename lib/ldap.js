...
var config = require('../config/ldap_config.js').config;
...
LDAPHandler.prototype = {
    ...
    get_users_for_roster: function(callback){
        var self = this;

        self.connect();

        self.ldap.simpleBind('cn=' + config.username + ',ou=Apps,dc=csh,dc=rit,dc=edu', config.password, function(msg_id, error){
            if(error == null){
                self.ldap.search('ou=Users,dc=csh,dc=rit,dc=edu', self.ldap.SUBTREE, "(uid=*)", "*", function(msg, error, data){
                    //console.log(data);
                    if(error == null){
                        if(data.length > 0){
                            // return users
